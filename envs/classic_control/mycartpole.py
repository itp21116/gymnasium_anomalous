...
class MyModCartPoleEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def __init__(self, case, max_episode_steps, when_anomaly_starts=30, render_mode: Optional[str] = None):
        self.gravity = 9.8
        self.masscart = 1.0
        self.masspole = 0.1
        ...
        #mary self.observation_space = spaces.Box(-1000000, 1000000, dtype=np.float32)
    ...
    def step(self, action):
        ...
        #print('******self.state : {}, type(self.state) : {}'.format(self.state, type(self.state)))  
        ...
        print(obs)
        print('##@@ newstate ##@@')
        newstate=(obs[0],obs[1],obs[2], obs[3])
        print(type(newstate))
        print(newstate)
        print('##@@ newstate float32 ##@@')
        print(np.float32(newstate))
        print('##@@ np.float32(obs) ##@@')
        print(np.float32(obs))'''
        return np.float32(obs), reward, terminated, False, {}
        #return np.array(newstate, dtype=np.float32), reward, terminated, False, {}
    ...
    def reset(
        ...
        *,
        ...
        #wtf self.state = self.np_random.uniform(low=-np.inf, high=np.inf, size=(4,))
        self.steps_beyond_terminated = None
        self.random_steps = []
        self.selected_sensors = np.zeros(self.observation_space.shape[0])    
        self.selected_sensors[: int(len(self.selected_sensors) / 3)] = 1
